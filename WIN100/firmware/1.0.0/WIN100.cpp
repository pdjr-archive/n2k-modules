...
void processSensorInputs() {
  ...
  if (now > deadline) {  
    ...
    // Docked sensor
    if (!DEBOUNCER.channelState(GPIO_W0_DOCKED_SENSOR)) {
      WINDLASS0.PGN128777v.setAnchorDockingStatus(N2kDD482_FullyDocked);
      WINDLASS0.PGN128777v.setRodeCounterValue(0);
    } else {
      WINDLASS0.PGN128777v.setAnchorDockingStatus(N2kDD482_NotDocked);
    }
    // Deployed sensor
    ...
    if ((!DEBOUNCER.channelState(GPIO_W0_DEPLOYING_SENSOR)) || (!DEBOUNCER.channelState(GPIO_W0_DEPLOYING_SENSOR))) {
      if (!DEBOUNCER.channelState(GPIO_W0_DEPLOYING_SENSOR)) WINDLASS0.PGN128777v.setWindlassMotionStatus(tN2kDD480_DeploymentOccurring);
      if (!DEBOUNCER.channelState(GPIO_W0_RETRIEVING_SENSOR)) WINDLASS0.PGN128777v.setWindlassMotionStatus(tN2kDD480_RetrievalOccurring);
    } else {
...
/**********************************************************************
 * Called by interrupt. Uses PULSE_DISTANCE to compute both windlass
 * rode speed and rode distance deployed.
 */
void onPulse() {
  static unsigned long lastPulseTime= 0UL;
  unsigned long now = millis();

  switch (WINDLASS0.PGN128777v.getWindlassMotionStatus()) {
    case N2kDD480_DeploymentOccurring:
      WINDLASS0.PGN128777v.setRodeCounterValue(WINDLASS0.PGN128777v.getRodeCounterValue() + PULSE_DISTANCE);
      break;
    case N2kDD480_RetrievalOccurring:
      WINDLASS0.PGN128777v.setRodeCounterValue(WINDLASS0.PGN128777v.getRodeCounterValue() - PULSE_DISTANCE);
      if (WINDLASS0.PGN128777v.getRodeCounterValue() < 0) WINDLASS0.PGN128777v.setRodeCounterValue(0);
      break;
    default:
      break;
  }
